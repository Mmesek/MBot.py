...
from MFramework.commands.command import (
    COMPILED_REGEX,
    aliasList,
    command_shortcuts,
    commands,
...
async def help(ctx: Context, command: str = None, *, language):
    ...
    desc = (
        tr(
            "commands.help.available_triggers",
            language,
            botid=ctx.bot.user_id,
    ...
    for cmd in filter(lambda x: not x.interaction, allowed_commands):
        ...
        if not _sig:
            _sig = " ".join(
                list(
                    f"[{i.name}]"
                    for i in filter(lambda x: "positional" in x.kind.lower(), cmd.arguments.values())
        ...
        try:
            alias = check_translation(
                f"commands.{cmd.name}.cmd_alias",
                language,
                list(aliasList.keys())[list(aliasList.values()).index(cmd.name)],
            )
            if alias != "":
    ...
    guild_commands = ""
    for cmd in filter(
        lambda x: x.interaction and x.guild == ctx.guild_id and " " not in cmd.name and not cmd.name.istitle(),
        allowed_commands,
    ):
