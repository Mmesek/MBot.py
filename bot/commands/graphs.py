...
async def graph(
    ctx: Context, graph="all", resample="Y", locator="Month", interval=4, *args, growth=False, language, **kwargs
):
    ...
    graphing.set_legend(
        ax,
        tr("commands.graph.growth", language),
        tr("commands.graph.memberCount", language),
        tr("commands.graph.dates", language),
        framealpha=0,
    )
    ...
    await ctx.reply(
        content=stats, attachments=[Attachment(file=img_str, filename=f"growth-{date.today()}.png")]
    )  # f"Took ~{truncate(time.time()-d,2)}s\n{truncate(s-f,2)}s to gather\n{truncate(d-sd,2)}s to sort\n{truncate(sd-s,2)}s to convert\n{truncate(f-b,2)}s to import stuff")


@register(group=Groups.ADMIN, interaction=False)
async def graph_infractions(
    ctx: Context,
    infraction_type="all",
    resample="D",
    locator="Week",
    ...
    **kwargs,
    ...
    graphing.set_legend(
        ax,
        tr("commands.graph.infractions", language),
        tr("commands.graph.infractionCount", language),
        "Dates (D/M)",
        framealpha=1,
    )
    fig.tight_layout()
    ...
    **kwargs,
    ...
    if total[word_or_phrase] == []:
        return await ctx.reply(
            f"Couldn't find specified word or phrase ({word_or_phrase}) within last fetchable {len(total_messages)} in <#{channel_id}>"
        )
    # for i in total:
    ...
    # Set Names
    graphing.set_legend(
        ax, tr("commands.graph.words", language), tr("commands.graph.wordCount", language), "Dates (D/M)"
    )
    fig.tight_layout()
    ...
    await ctx.reply(
        f"Found {len(total[word_or_phrase])} messages containing `{word_or_phrase}` within {len(total_messages)} of total fetched messages. (Took {truncate(s-f,2)}s to fetch them)",
        attachments=[Attachment(file=img_str, filename=f"growth-{date.today()}.png")],
    )
