...
async def show(ctx: Context, *, language):
    ...
    dates += tr(
        "commands.profile.datesServer",
        language,
        server=datetime.fromisoformat(ctx.member.joined_at).strftime("%Y-%m-%d %H:%M:%S"),
    )
    ...
    if ctx.member.premium_since:
        dates += tr(
            "commands.profile.datesBoostStart",
            language,
            boost=datetime.fromisoformat(ctx.member.premium_since).strftime("%Y-%m-%d %H:%M:%S"),
        )
...
async def language(ctx: Context, new_language, *, language):
    ...
    try:
        ...
        return await ctx.message(
            ctx.channel, tr("commands.profile.notFoundLanguage", language, language=new_language)
        )  # f"Couldn't find language {new_language}")
...
async def timezone(ctx: Context, timezone, *, language):
    ...
    if any(timezone.lower() == i.lower() for i in pytz.all_timezones):
        ...
        return await ctx.message(
            ctx.channel_id, tr("commands.profile.notFoundTimezone", language, timezone=timezone)
        )  # f"Couldn't find Timezone {timezone}.")
    s.merge(c)
    s.commit()
...
async def region(ctx: Context, region, *, language):
    ...
    try:
        ...
        return await ctx.message(
            ctx.channel, tr("commands.profile.notFoundRegion", language, region=region)
        )  # f"Couldn't find region {region}")
    c.Region = region
    s.merge(c)
    s.commit()
