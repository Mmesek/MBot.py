...
async def answer(ctx: Context, answer: str) -> str:
    ...
    key = (
        ...
        .filter(
            Answers_Registered.value.in_([lower_answer, normalized_answer.upper()]),
            Answers_Registered.timestamp <= datetime.utcnow(),
        )
        .first()
    )
    if key:
        ...
        previous_answer = (
            session.query(Answers_Puzzle)
            .filter(Answers_Puzzle.key == key, Answers_Puzzle.user_id == ctx.user_id)
            .first()
        )
        if previous_answer:
        ...
        session.commit()
        all_answers = (
            session.query(sa.func.count(Answers_Registered.key))
            .filter(Answers_Registered.puzzle == _puzzle)
            .group_by(Answers_Registered.key)
            .all()
        )
        all_answered = (
            session.query(sa.func.count(Answers_Puzzle.key))
            .filter(Answers_Puzzle.user_id == ctx.user_id, Answers_Puzzle.puzzle == _puzzle)
            .first()
        )
        ...
        return (
            "Congratulations, you have solved this puzzle! You'll receive your reward(s) after the event ends (or sooner, it reeally depends) based on first-come first-served basis."
            + remaining
        )
