...
async def nitro(ctx: Context, hex_color: str = None, name: str = None, emoji: str = None) -> str:
    ...
    if hex_color:
        try:
            ...
            return "Color has to be provided as a hexadecimal value (between 0 to F for example `#012DEF`) not `" + str(
                ex
            ).split(": '")[-1].replace("'", "`")
    else:
        color = None
    ...
    c = _roles.filter(db.Role.with_setting(db.types.Setting.Custom, ctx.user_id)).first()
    ...
    if c and role and role.id != c.id or not c:
        ...
        role = await ctx.bot.create_guild_role(
            ...
            False,
            mentionable=False,
            reason="Created Role for Nitro user " + ctx.member.user.username,
            **emoji,
        )
    ...
    if _id:
        await ctx.bot.execute_webhook(
            ...
            content="{user} {state} <@&{role}> with name {name} and color {color}{emoji}".format(
                ...
                name=role.name,
                color=role.color,
                emoji=f" and icon {emoji}" if emoji else "",
            ),
            username="Nitro Role",
        )
...
from MFramework import Bot, Guild_Member_Update, Snowflake, onDispatch
...
async def new_booster(ctx: Bot, user_id: Snowflake, guild_id: Snowflake):
    ...
    message = " ".join(
        [
            choice(greeting).format(user=user_id),
            choice(color),
            choice(fine_print),
            choice(if_interest).format(bot=ctx.username, cmd=cmd_line),
            choice(ending),
            choice(note),
        ]
    )
...
async def end_booster(ctx: Bot, user_id: Snowflake, guild_id: Snowflake):
    ...
    s = ctx.db.sql.session()
    c = db.Role.filter(s, server_id=guild_id).filter(db.Role.with_setting(db.types.Setting.Custom, user_id)).first()
    if c:
