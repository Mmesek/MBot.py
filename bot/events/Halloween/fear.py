...
async def summon(
    ...
    **kwargs,
    ...
    avg_army_size = (
        session.query(sa.func.avg(items.Inventory.quantity), items.Inventory.item_id)
        .filter(items.Inventory.item_id.in_(list(monster_ids.keys())))
        .group_by(items.Inventory.item_id)
        .all()
    )
...
async def raid(ctx: Context, boss: Alive_Bosses, *, session: sa.orm.Session, **kwargs):
    ...
    boss = Bosses.get(boss.replace(" ", "_"))
...
async def convert_fear(ctx: Context):
    ...
    inventories: List[items.Inventory] = (
        session.query(items.Inventory).filter(items.Inventory.item_id.in_([16, 18, 20, 21, 22, 29])).all()
    )
    ...
    for inv in inventories:
        ...
        t = u.transfer(ctx.guild_id, u, [inv], [rf], False, True)
        session.add(t)
    session.commit()
