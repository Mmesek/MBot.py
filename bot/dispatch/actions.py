        ...
        if snippets:
            ...
            for snippet in snippets:
                ...
                if cmd:
                    ...
                    _ = await stashed(ctx, cmd, args, search, detailed=False, show_all=False)
                    if _:
                        r.append(_)
            if r:
                await ctx.reply("\n".join(r))
...
async def blocked_words(self: Bot, data: Message) -> bool:
    BLACKLISTED_WORDS = self.cache[
        data.guild_id
    ].blacklisted_words  # re.compile(r"") #TODO: Source cached from Database!
