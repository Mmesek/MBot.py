...
import gpxpy
import httpx
from MFramework import Bot, Embed, Message, Message_Reference, onDispatch
...
async def message_create(bot: Bot, data: Message):
    ...
    for attachment in data.attachments:
        if attachment.filename.endswith("gpx"):
            ...
            embeds.append(
                Embed(title=attachment.filename, timestamp=gpx.time, url=attachment.url)
    ...
    await bot.create_message(
        data.channel_id, f"Processed in {e - s:.2}s", embeds=embeds, message_reference=Message_Reference(data.id)
    )
